...
<head>
    ...
    <style>
        ...
        .container { background: white; padding: 30px; border-radius: 16px; box-shadow: 0 4px 20px rgba(0,0,0,0.06); width: 90%; max-width: 380px; text-align: center; }
        
        /* Typography */
        h1 { font-size: 22px; margin-bottom: 8px; color: #111; }
        p.subtitle { font-size: 14px; color: #007aff; font-weight: 600; margin-top: 0; margin-bottom: 20px; }
        
        /* Status and Input elements */
        .status-box { background: #eef2f7; padding: 12px; border-radius: 8px; margin-bottom: 25px; font-weight: 500; font-size: 14px; color: #555; }
        .status-box span { color: #2ecc71; font-weight: bold; }
        .input-group { text-align: left; margin-bottom: 20px; }
        label { display: block; margin-bottom: 8px; font-size: 14px; color: #555; font-weight: 500; }
        input[type="number"] { width: 100%; padding: 12px; border: 1px solid #ddd; border-radius: 8px; font-size: 16px; box-sizing: border-box; text-align: center; }
        
        /* Buttons */
        button { width: 100%; padding: 14px; border: none; border-radius: 8px; font-size: 16px; font-weight: 600; cursor: pointer; transition: background 0.2s; margin-bottom: 12px; }
        .btn-start { background: #007aff; color: white; }
        .btn-start:hover { background: #0062cc; }
        .btn-stop { background: #ff3b30; color: white; }
        .btn-stop:hover { background: #d32f2f; }
        
        /* Animated Progress Bar */
        .progress-container { width: 100%; background-color: #eef2f7; border-radius: 10px; margin: 20px 0 5px 0; display: none; overflow: hidden; }
        .progress-bar { width: 1%; height: 12px; background-color: #007aff; border-radius: 10px; transition: width 0.05s linear; }
        .progress-text { font-size: 13px; font-weight: 600; color: #555; margin-bottom: 15px; display: none; }
    </style>
</head>
<body>
    <div class="container">
        <h1>Robot Control Center</h1>
        <p class="subtitle">Blue Stamp Engineering Project</p>
        ...
        <div class="input-group">
            ...
            <input type="number" id="duration" min="1" max="3600" value="15">
        </div>
        
        <button class="btn-start" onclick="sendTimer()">Start Cleaning</button>
        
        <div class="progress-container" id="pContainer">
            <div class="progress-bar" id="pBar"></div>
        </div>
        <div class="progress-text" id="pText">Uploading: 1%</div>
        
        <button class="btn-stop" onclick="stopRobot()">Emergency Stop</button>
    </div>

    <script>
        // Function to handle the start command and animation
        function sendTimer() {
            var timeVal = document.getElementById('duration').value;
            ...
            pText.style.color = '#555'; 
            pBar.style.width = '1%';
            
            // Fast loading animation from 1% to 100%
            ...
            var animationInterval = setInterval(function() {
                if (currentPercent >= 100) {
                    ...
                    pText.innerText = 'Robot Accepted'; 
                    pText.style.color = '#2ecc71';
                    
                    statusEl.innerText = 'Cleaning active (' + timeVal + 's)'; 
                    statusEl.style.color = '#2ecc71';
                    
                    // Send actual fetch request to the ESP8266 server
                    fetch('/set?time=' + timeVal).catch(err => { console.log('Fetch error'); });
                } else {
                    currentPercent++; 
                    pBar.style.width = currentPercent + '%'; 
                    pText.innerText = 'Uploading Command: ' + currentPercent + '%';
                }
            }, 12); // 12ms interval for a fast, dynamic effect
        }

        // Function to handle the emergency stop command
        function stopRobot() {
            var statusEl = document.getElementById('status');
            
            // Hide progress bar on stop
            document.getElementById('pContainer').style.display = 'none'; 
            document.getElementById('pText').style.display = 'none';
            
            statusEl.innerText = 'Standby / Stopped'; 
            statusEl.style.color = '#ff3b30';
            
            fetch('/stop').catch(err => {});
        }
    </script>
</body>
</html>
